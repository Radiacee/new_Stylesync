      ...
      <div className="flex items-center justify-between mb-4 border-b border-white/5 pb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 dark:text-slate-500 dark:text-slate-400">Proof of style transformation and alignment</p>
          </div>
        </div>
        <Sparkles className="w-5 h-5 text-brand-600 dark:text-brand-300 animate-pulse" />
      </div>

      {/* Explanation Box */}
      <div className={`rounded-lg bg-gradient-to-r ${meta.bgGradient} border border-white/5 p-4`}>
        <div className="flex items-start gap-2.5 mb-3">
          <Info className="w-4 h-4 text-brand-600 dark:text-brand-300 mt-0.5" />
          <h4 className="text-xs font-semibold text-brand-600 dark:text-brand-300 uppercase tracking-wider">
            Why is this {stylePreset}?
          </h4>
        </div>

        <div className="prose prose-invert prose-xs sm:prose-sm max-w-none text-slate-800 dark:text-slate-300 leading-relaxed">
          <ReactMarkdown
            ...
            components={{
              ...
              p: ({ children }: any) => <p className="text-xs text-slate-800 dark:text-slate-300 my-1">{children}</p>,
              strong: ({ children }: any) => <strong className="font-semibold text-slate-900 dark:text-white">{children}</strong>,
              code: ({ children }: any) => (
                <code className="px-1.5 py-0.5 rounded bg-white/10 text-brand-600 dark:text-brand-300 font-mono text-[0.9em] break-all">
                  {children}
                </code>
              )
            }}
