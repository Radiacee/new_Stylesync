        ...
        <ReactMarkdown
          ...
          components={{
            code({ node, inline, className, children, ...props }: any) {
                  ...
                  <div className="relative group rounded-xl overflow-hidden border border-white/10 my-6 bg-[#1E1E1E]">
                    <div className="flex items-center justify-between px-4 py-2 bg-white/5 border-b border-white/10">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-slate-400 font-mono uppercase tracking-wider ml-2">
                          {language}
                        </span>
                      </div>
                      <button
                        onClick={() => handleCopyCode(codeString, blockId)}
                        className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-white/5 hover:bg-white/10 text-slate-300 text-xs transition-colors"
                      >
              ...
              return (
                <code {...props} className="px-1.5 py-0.5 rounded-md bg-white/10 text-brand-300 font-mono text-[0.9em]">
                  {children}
                </code>
              );
            },
            h1: ({children}) => <h1 className="text-2xl font-bold text-white mb-4 mt-6 flex items-center gap-2"><span className="text-brand-500">#</span> {children}</h1>,
            h2: ({children}) => <h2 className="text-xl font-bold text-slate-100 mb-3 mt-6 flex items-center gap-2"><span className="text-brand-400">##</span> {children}</h2>,
            h3: ({children}) => <h3 className="text-lg font-semibold text-slate-200 mb-2 mt-5">{children}</h3>,
            ...
            blockquote: ({children}) => (
              <blockquote className="border-l-2 border-brand-500/50 bg-brand-500/5 py-2 px-4 rounded-r-lg my-4 italic text-slate-300">
                {children}
              </blockquote>
            )
          }}
