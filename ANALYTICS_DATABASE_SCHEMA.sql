...
-- This schema supports collecting all paraphrase results for insights and suggestions

-- 1. Paraphrase Analytics Table
-- Stores paraphrase results with style settings and verification scores
CREATE TABLE IF NOT EXISTS public.paraphrase_analytics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  
  -- Optional: ID of saved style profile (for duplicate detection)
  profile_id TEXT,
  
  -- Optional: Name of saved style profile (for display)
  profile_name TEXT,
  ...
  -- Performance Metrics
  verification_score REAL NOT NULL CHECK (verification_score >= 0 AND verification_score <= 100),
...
CREATE INDEX IF NOT EXISTS idx_analytics_profile_id ON public.paraphrase_analytics(profile_id);
CREATE INDEX IF NOT EXISTS idx_analytics_user_profile ON public.paraphrase_analytics(user_id, profile_id);
...
-- Sample query to get top suggestions
-- SELECT * FROM public.analytics_suggestions LIMIT 10;

-- Sample query to get suggestions for a specific tone
